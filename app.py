        ...
        with db_lock:
            try:
                ...
                """, (
                    ...
                    ticket_data.get('status', 'Open'), ticket_data.get('assigned_to', ''),
                    ticket_data['category'], ticket_data.get('subcategory', ''),
                    datetime.now().isoformat(), due_date.isoformat(), user_name, user_id,
                    ticket_data.get('tags', ''), ticket_data.get('estimated_hours', 0),
                    ticket_data.get('company_id', ''), ticket_data.get('source', 'Manual')
                ))
    ...
    def get_ticket_statistics(self, user_id: int, permissions: Dict, user_name: str) -> Dict:
        with db_lock:
            try:
                ...
                if permissions.get('can_view_all_tickets', False):
                    cursor.execute("SELECT status, COUNT(*) as count FROM tickets WHERE status != 'Deleted' GROUP BY status")
                else:
                    cursor.execute("""
                        SELECT status, COUNT(*) as count FROM tickets 
                        WHERE (reporter = ? OR assigned_to = ?) AND status != 'Deleted' GROUP BY status
                    """, (user_name, user_name))
...
def require_auth(permission: str = None) -> bool:
    ...
    with st.form("edit_user_form"):
        ...
        with col2:
            role = st.selectbox("Role*", ["User", "Agent", "Manager", "Admin"], index=["User", "Agent", "Manager", "Admin"].index(user['role']))
            department = st.text_input("Department", value=user.get('department', ''))
            if company_options:
                current_company = next((i for i, comp in enumerate(company_options) if comp.startswith(user.get('company_id', ''))), 0)
        ...
        with perm_col1:
            can_create_users = st.checkbox("Can Create Users", value=user['permissions'].get('can_create_users', False))
            can_deactivate_users = st.checkbox("Can Deactivate Users", value=user['permissions'].get('can_deactivate_users', False))
            can_reset_passwords = st.checkbox("Can Reset Passwords", value=user['permissions'].get('can_reset_passwords', False))
            can_manage_tickets = st.checkbox("Can Manage Tickets", value=user['permissions'].get('can_manage_tickets', False))
        with perm_col2:
            can_view_all_tickets = st.checkbox("Can View All Tickets", value=user['permissions'].get('can_view_all_tickets', False))
            can_delete_tickets = st.checkbox("Can Delete Tickets", value=user['permissions'].get('can_delete_tickets', False))
            can_export_data = st.checkbox("Can Export Data", value=user['permissions'].get('can_export_data', False))
        ...
        if submitted:
            if first_name and last_name:
                user_data = {'first_name': first_name, 'last_name': last_name, 'phone': phone, 'role': role, 'department': department, 'company_id': company_id, 'can_create_users': can_create_users, 'can_deactivate_users': can_deactivate_users, 'can_reset_passwords': can_reset_passwords, 'can_manage_tickets': can_manage_tickets, 'can_view_all_tickets': can_view_all_tickets, 'can_delete_tickets': can_delete_tickets, 'can_export_data': can_export_data}
...
def show_sidebar():
    with st.sidebar:
        ...
        if st.session_state.user:
            ...
            st.markdown(f'<div style="text-align: center; margin-bottom: 1rem;"><strong>{user["full_name"]}</strong><br><span class="{role_class}">{user["role"]}</span><br><small>{user["department"]} | {user["company_id"]}</small></div>', unsafe_allow_html=True)
            st.markdown("---")
...
def generate_csv_export(tickets):
    ...
    writer.writerow(['ID', 'Title', 'Description', 'Priority', 'Status', 'Assigned To', 'Category', 'Subcategory', 'Created Date', 'Updated Date', 'Due Date', 'Reporter', 'Tags', 'Estimated Hours', 'Actual Hours', 'Company ID', 'Source'])
    for ticket in tickets:
        writer.writerow([ticket['id'], ticket['title'], ticket['description'], ticket['priority'], ticket['status'], ticket['assigned_to'], ticket['category'], ticket['subcategory'], format_date(ticket['created_date']), format_date(ticket['updated_date']), format_date(ticket['due_date']), ticket['reporter'], ticket['tags'], ticket['estimated_hours'], ticket['actual_hours'], ticket['company_id'], ticket['source']])
...
def show_dashboard():
    ...
    st.markdown(f'<div class="main-header"><h1>🎫 FlowTLS SYNC+ Dashboard</h1><p>Welcome back, {user_name}! | Role: <strong>{user["role"]}</strong> | Department: {user["department"]} | Company: {user["company_id"]}</p></div>', unsafe_allow_html=True)
    ...
    with col1:
        if stats:
            ...
            if chart_stats:
                fig = px.pie(values=list(chart_stats.values()), names=list(chart_stats.keys()), title="Tickets by Status", color_discrete_sequence=['#3b82f6', '#8b5cf6', '#10b981', '#6b7280', '#f59e0b'])
    ...
    with col2:
        ...
        for ticket in tickets:
            ...
            st.markdown(f'<div class="ticket-card"><h4 style="color: #1f2937 !important;">{overdue_indicator} #{ticket["id"]} - {ticket["title"]}</h4><p style="color: #4b5563 !important;">{ticket["description"][:80]}...</p><div><span class="{priority_class}">{ticket["priority"]}</span><span class="{status_class}">{ticket["status"]}</span><span style="margin-left: 10px; color: #4b5563;">👤 {ticket["assigned_to"]} | 🏢 {ticket["company_id"]}</span></div></div>', unsafe_allow_html=True)
...
def show_tickets_page():
    ...
    with col3:
        if st.button("🔄 Refresh", use_container_width=True):
            st.rerun()
    with col4:
        filter_status = st.selectbox("Filter by Status", ["All", "Open", "In Progress", "Resolved", "Closed", "On Hold"])
    with col5:
        if user['permissions'].get('can_export_data', False):
            if st.button("📊 Export CSV", use_container_width=True):
                ...
                st.download_button(label="Download CSV", data=csv_data, file_name=f"flowtls_tickets_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", mime="text/csv")
    
    tickets = ticket_service.get_all_tickets(user['id'], user['permissions'], user_name)
    if search_term:
        tickets = [t for t in tickets if search_term.lower() in t['title'].lower() or search_term.lower() in t['description'].lower() or search_term.lower() in t['tags'].lower()]
...
def display_professional_ticket_list(tickets, user):
    ...
    for ticket in tickets:
        ...
        with st.container():
            ...
            with col1:
                overdue_indicator = "🔥 " if ticket['is_overdue'] else ""
                st.markdown(f'<div class="ticket-card {overdue_class}"><h4 style="color: #1f2937 !important; margin-bottom: 0.75rem;">{overdue_indicator}#{ticket["id"]} - {ticket["title"]}</h4><p style="color: #4b5563 !important; margin-bottom: 0.75rem;">{ticket["description"][:150]}{"..." if len(ticket["description"]) > 150 else ""}</p><div style="margin-bottom: 0.5rem;"><span class="{priority_class}">{ticket["priority"]}</span><span class="{status_class}">{ticket["status"]}</span><span style="margin-left: 10px; color: #4b5563;">📁 {ticket["category"]}</span><span style="margin-left: 10px; color: #4b5563;">👤 {ticket["assigned_to"]}</span><span style="margin-left: 10px; color: #4b5563;">🏢 {ticket["company_id"]}</span></div><div style="font-size: 0.875rem; color: #6b7280;"><span>📅 Created: {format_date(ticket["created_date"])}</span><span style="margin-left: 15px;">⏰ Due: {format_date(ticket["due_date"])}</span><span style="margin-left: 15px;">👨‍💼 Reporter: {ticket["reporter"]}</span><span style="margin-left: 15px;">📧 Source: {ticket["source"]}</span></div></div>', unsafe_allow_html=True)
            with col2:
...
def show_new_ticket_page():
    ...
    with st.form("new_ticket_form"):
        ...
        with col1:
            ...
            category = st.selectbox("Category*", ["General", "Technical Support", "Bug Report", "Feature Request", "Security", "Performance", "Integration", "Configuration", "Enhancement", "Infrastructure", "Email System"])
        ...
        if submitted:
            if title and description and priority and category:
                ticket_data = {'title': title, 'description': description, 'priority': priority, 'status': status, 'assigned_to': assigned_to, 'category': category, 'subcategory': subcategory, 'tags': tags, 'estimated_hours': estimated_hours, 'company_id': company_id, 'source': 'Manual'}
...
def show_users_page():
    ...
    if search_term:
        users = [u for u in users if search_term.lower() in u['username'].lower() or search_term.lower() in u['full_name'].lower() or search_term.lower() in u['email'].lower()]
    ...
    for user in users:
        ...
        with st.container():
            ...
            with col1:
                ...
                st.markdown(f'<div class="user-card {user_class}"><h4 style="color: #1f2937 !important; margin-bottom: 0.75rem;">👤 {user["full_name"]} (@{user["username"]}) {status_indicator}</h4><p style="color: #4b5563 !important; margin-bottom: 0.75rem;">📧 {user["email"]} | 📞 {user["phone"]} | 🏢 {user["company_id"]}</p><div style="margin-bottom: 0.5rem;"><span class="{role_class}">{user["role"]}</span><span style="margin-left: 10px; color: #4b5563;">🏢 {user["department"]}</span></div><div style="font-size: 0.875rem; color: #6b7280;"><span>📅 Created: {format_date(user["created_date"])}</span><span style="margin-left: 15px;">⏰ Last Login: {format_date(user["last_login_date"])}</span><span style="margin-left: 15px;">👨‍💼 Created by: {user["created_by"]}</span></div></div>', unsafe_allow_html=True)
            with col2:
    ...
    with st.form("new_user_form"):
        ...
        if submitted:
            if username and first_name and last_name and email and password:
                user_data = {'username': username, 'first_name': first_name, 'last_name': last_name, 'email': email, 'password': password, 'phone': phone, 'role': role, 'department': department, 'company_id': company_id, 'can_create_users': can_create_users, 'can_deactivate_users': can_deactivate_users, 'can_reset_passwords': can_reset_passwords, 'can_manage_tickets': can_manage_tickets, 'can_view_all_tickets': can_view_all_tickets, 'can_delete_tickets': can_delete_tickets, 'can_export_data': can_export_data}
...
class DatabaseManager:
    ...
    def _create_default_users(self, cursor):
        users = [
            {
                'username': 'admin', 'email': 'admin@flowtls.com', 'password': 'admin123',
                'first_name': 'System', 'last_name': 'Administrator', 'role': 'Admin',
                'department': 'IT', 'phone': '+1-555-0001', 'company_id': 'FLOWTLS001',
                'can_create_users': 1, 'can_deactivate_users': 1, 'can_reset_passwords': 1,
                'can_manage_tickets': 1, 'can_view_all_tickets': 1, 'can_delete_tickets': 1,
                'can_export_data': 1
            },
            {
                'username': 'jsmith', 'email': 'john.smith@flowtls.com', 'password': 'password123',
                'first_name': 'John', 'last_name': 'Smith', 'role': 'Manager',
                'department': 'Support', 'phone': '+1-555-0002', 'company_id': 'FLOWTLS001',
                'can_create_users': 0, 'can_deactivate_users': 0, 'can_reset_passwords': 0,
                'can_manage_tickets': 1, 'can_view_all_tickets': 1, 'can_delete_tickets': 0,
                'can_export_data': 1
            },
            {
                'username': 'achen', 'email': 'alice.chen@flowtls.com', 'password': 'password123',
                'first_name': 'Alice', 'last_name': 'Chen', 'role': 'Agent',
                'department': 'Technical', 'phone': '+1-555-0003', 'company_id': 'FLOWTLS001',
                'can_create_users': 0, 'can_deactivate_users': 0, 'can_reset_passwords': 0,
                'can_manage_tickets': 1, 'can_view_all_tickets': 1, 'can_delete_tickets': 0,
                'can_export_data': 0
            },
            {
                'username': 'sjohnson', 'email': 'sarah.johnson@flowtls.com', 'password': 'password123',
                'first_name': 'Sarah', 'last_name': 'Johnson', 'role': 'User',
                'department': 'Operations', 'phone': '+1-555-0005', 'company_id': 'CLIENT001',
                'can_create_users': 0, 'can_deactivate_users': 0, 'can_reset_passwords': 0,
                'can_manage_tickets': 0, 'can_view_all_tickets': 0, 'can_delete_tickets': 0,
                'can_export_data': 0
            }
        ]
